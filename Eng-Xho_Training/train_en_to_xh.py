...
def test_model(model, tokenizer, device, test_sentences, label="UNTRAINED"):
    ...
    for text in test_sentences:
        ...
        with torch.no_grad():
            forced_bos_token_id = tokenizer.convert_tokens_to_ids("xho_Latn")
...
def main():
    ...
    model.generation_config.forced_bos_token_id = tokenizer.convert_tokens_to_ids("xho_Latn")
    model.generation_config.max_length = 128
    model.generation_config.num_beams = 5  # Beam search for better quality
    
    # Set decoder_start_token_id if not set
    if model.config.decoder_start_token_id is None:
        model.config.decoder_start_token_id = tokenizer.convert_tokens_to_ids("xho_Latn")
    
    # Test the base model
    test_sentences_before = [
        "Hello, how are you?",
        "The patient has a fever and headache."
    ]
    test_model(model, tokenizer, device, test_sentences_before, label="UNTRAINED")
    
    tokenizer.src_lang = "eng_Latn"
    tokenizer.tgt_lang = "xho_Latn"
    ...
    test_sentences_after = [
        ...
        "I have been having headaches for three days.",
