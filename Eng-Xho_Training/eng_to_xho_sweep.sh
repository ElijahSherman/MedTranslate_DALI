...
STDERR_LOG="${LOG_DIR}/en_to_xh_sweep.e"
...
# --- STATE MANAGEMENT ---
STATE_FILE="sweep_state_eng_to_xho.txt"
if [ ! -f "$STATE_FILE" ]; then
    echo "0" > "$STATE_FILE"
    echo "Initialized state file for Eng to Xho sweep."
fi
...
for (( i=${START_INDEX}; i<${TOTAL_EXPERIMENTS}; i++ )); do
    ...
    OUTPUT_DIR="finetuned_models/en_to_xh_sweep/${RUN_NAME}"
...
echo ""
echo "=== English to isiXhosa Hyperparameter Sweep Complete ==="