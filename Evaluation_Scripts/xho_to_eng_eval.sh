...
# This script evaluates all the models trained in the isiXhosa to English hyperparameter sweep

# --- Log & Result File Configuration ---
LOG_DIR="${PWD}/eval_logs"
OUTPUT_DIR_BASE="outputs/xho_to_eng_evaluation"
RESULTS_FILE="${OUTPUT_DIR_BASE}/xh_to_en_sweep_results.csv"

mkdir -p "$OUTPUT_DIR_BASE"
exec >> "${LOG_DIR}/blocker_bwd.o" 2>> "${LOG_DIR}/blocker_bwd.e"
...
for (( i=0; i<${TOTAL_EXPERIMENTS}; i++ )); do
    ...
    MODEL_PATH="finetuned_models/xh_to_en_sweep/${RUN_NAME}"
    ...
    SOURCE_FILE="${PWD}/data-bin/dev.xh"
    REFERENCE_FILE="${PWD}/data-bin/dev.en"
    PREDICTION_FILE="${OUTPUT_DIR_BASE}/${RUN_NAME}.pred.en"
...
echo ""
echo "=== All isiXhosa to English Translation Model Evaluations Complete ==="